...
impl Discovery {
    ...
    pub async fn new(
        ...
        info!(
            ...
            "tcp6" => ?local_enr.tcp6(),
            "udp6" => ?local_enr.udp6(),
            "quic4" => ?local_enr.quic4(),
            "quic6" => ?local_enr.quic6()
        );

        // convert the keypair into an ENR key
        let enr_key: CombinedKey = CombinedKey::from_libp2p(local_key).map_err(Error::msg)?;
        ...
        for bootnode_enr in config.boot_nodes_enr.clone() {
            ...
            debug!(
                ...
                "ip" => ?bootnode_enr.ip4(),
                "udp4" => ?bootnode_enr.udp4(),
                "tcp4" => ?bootnode_enr.tcp4(),
    ...
    pub fn update_enr_bitfield(&mut self, subnet: Subnet, value: bool) -> Result<()> {
        ...
        match subnet {
            Subnet::Attestation(id) => {
                ...
                let mut current_bitfield = local_enr
...
mod tests {
    ...
    async fn build_discovery() -> Discovery {
        ...
        let enr: Enr = build_enr(&enr_key, &config, &EnrForkId::default()).unwrap();
